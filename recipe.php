    <?php include 'nav.php'; ?>
    <div>
        <script src="saveRecipeAjax.js" defer></script>
      <?php
	  ...
	  if (isset($_GET['id']) && $dao->getRecipe($_GET['id'])) {
	      ...
	      if ($dao->getSavedRecipe($_SESSION['user_id'], $id)) {
                echo '<ul><li><button id="remove_button">Unsave</button></li></ul>';
	      } else { 
                echo '<ul><li><button id="save_button">Save</button></li></ul>';
	      }
	      if ($recipe['user_id'] === $_SESSION['user_id']) {
                echo '<ul><li><button class="spaced_button danger-bg"><a href="remove_recipe.php?id=' . $recipe['id'] . '">Delete</a></button></li></ul>';
              }
	      echo '</li></ul>';
            } else {
